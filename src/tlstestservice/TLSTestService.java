...
//	TLS currentTLS = new TLS10();
	...
	public String receiveMessages() throws Exception {
		...
		while (input.available() > 0) {
			...
			case TLS.CONTENT_TYPE_HANDSHAKE:
				...
				if(DEBUG) System.out.println("Adding to handshake buffer (incoming message): " + Utils.bytesToHexString(handshake.getBytes()));
				...
				case TLS.HANDSHAKE_MSG_TYPE_CLIENT_KEY_EXCHANGE:
					...
					switch (cipherSuite.keyExchange) {
						...
						case CipherSuite.ALG_DHE_RSA:
						    ...
						    if(DEBUG) System.out.println("premaster_secret: " + Utils.bytesToHexString(premaster_secret_server));
	...
	void sendMessage(byte type, byte[] msg) throws Exception {
		...
		if (ccs_out) {
			if (DEBUG) System.out.println("Sending record (before encryption): " + Utils.bytesToHexString(record.getBytes()));
	...
	void sendHandshakeMessage(HandshakeMsg msg, boolean updateHash) throws Exception {
		if(updateHash) {
			if(DEBUG) System.out.println("Adding to handshake buffer (outgoing message): " + Utils.bytesToHexString(msg.getBytes()));
